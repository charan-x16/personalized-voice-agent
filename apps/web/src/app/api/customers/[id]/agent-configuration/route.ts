...
const MAX_AGENT_CONFIGURATION_UPDATE_BYTES = 16 * 1024;
...
export async function PATCH(
  ...
  try {
    body = await readJsonObject(request, MAX_AGENT_CONFIGURATION_UPDATE_BYTES);
  } catch (error) {
  ...
  const update = parseAgentConfigurationUpdatePayload(body);
  if (!update) {
    return bffErrorResponse(
      "Supply a valid expected revision, at least one agent field, and no unknown fields.",
      400,
    );
  }
  ...
  try {
    upstream = await requestBackend(
      `/v1/customers/${encodeURIComponent(id)}/agent-configuration`,
      {
        method: "PATCH",
        token,
